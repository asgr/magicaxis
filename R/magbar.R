magbar = function(position='topright', range=c(0,1), orient='v', log=FALSE, col=hcl.colors(21), scale=c(1/4,1/20),
         inset=1/40, labN=5, title='', titleshift=0, centrealign='rb', clip='', cex=1, ...){
  usercoord = par()$usr
  xlogcheck = FALSE
  ylogcheck = FALSE
  if (par()$xlog) {
  ...
  if (orient == 'h') {
    ...
    if (length(grep('bottom', position)) > 0) {
      align = 'lt'
      yb = ylo + ydiff * inset
      yt = ylo + ydiff * inset + ydiff * scale[2]
    }
    if (length(grep('top', position)) > 0) {
      align = 'rb'
      yb = yhi - ydiff * inset - ydiff * scale[2]
      yt = yhi - ydiff * inset
    }
    if (length(grep('left', position)) > 0) {
      xl = xlo + xdiff * inset
      xr = xlo + xdiff * inset + xdiff * scale[1]
    }
    if (length(grep('right', position)) > 0) {
      xl = xhi - xdiff * inset - xdiff * scale[1]
      xr = xhi - xdiff * inset
    }
  }
  ...
  if (orient == 'v') {
    ...
    if (length(grep('bottom', position)) > 0) {
      yb = ylo + ydiff * inset
      yt = ylo + ydiff * inset + ydiff * scale[1]
    }
    if (length(grep('top', position)) > 0) {
      yb = yhi - ydiff * inset - ydiff * scale[1]
      yt = yhi - ydiff * inset
    }
    if (length(grep('left', position)) > 0) {
      align = 'rb'
      xl = xlo + xdiff * inset
      xr = xlo + xdiff * inset + xdiff * scale[2]
    }
    if (length(grep('right', position)) > 0) {
      align = 'lt'
      xl = xhi - xdiff * inset - xdiff * scale[2]
      xr = xhi - xdiff * inset
    }
  }
